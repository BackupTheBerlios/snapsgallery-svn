		...
		<table cellpadding="0" cellspacing="0" border="0" style="width: 100%;">
			...
			<tr>
				...
				if (is_writable('../Snaps!.config.php')) {
					...
					echo STR_ERR.' Please chmod the config file (Snaps!.config.php) to 777.';
				}
				?>
				</td>
			</tr>
...
<?php
	...
	if (!file_exists('../Snaps!.config.php')) {
		echo '<p>'.STR_ERR.' The config file (Snaps!.config.php) was not found. Please correct the error and run this script again.</p>';
	} else {
		if (is_writable('../Snaps!.config.php')) {
...
<?php
				...
				$cfgFile = file_get_contents('../Snaps!.config.php');
				...
				$fp = fopen('../Snaps!.config.php', 'w');
				if (!$fp) {
					echo ' '.STR_ERR.' There was a problem opening the config file (Snaps!.config.php) for writing. Please chmod the config file to 777.</p>';
				} else {
					if (fwrite($fp, $cfgFile) === FALSE) {
						echo ' '.STR_ERR.' Could not write the configuration information to the config file (Snaps!.config.php). Please copy the text below into the config file.</p><br /><pre>'.$cfgFile.'</pre>';
					} else {
						$err = 0;
?>
				<table cellpadding="3" cellspacing="0" border="0" style="width: 100%;">
...
<?php
			...
			echo '<p>'.STR_ERR.' The config file (Snaps!.config.php) is not writable. Please chmod the config file to 777 and run this script again.</p>';
		}
	}
?>
			</div>
